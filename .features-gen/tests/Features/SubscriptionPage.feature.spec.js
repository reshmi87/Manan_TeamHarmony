...
test.describe('Subscription page for unauthenticated users', () => {
  ...
  test('Verifying that a signed-up user can subscribe to Premium', async ({ Given, page, When, Then }) => { 
    await Given('The user signs in using a valid username and password from the Signin window', null, { page }); 
    await When('The user attempts to upgrade to a Premium subscription'); 
    await Then('The user should be redirected to the checkout page'); 
  });
...
const bddFileData = [ // bdd-data-start
  ...
  {"pwTestLine":24,"pickleLine":20,"tags":[],"steps":[{"pwStepLine":25,"gherkinStepLine":21,"keywordType":"Context","textWithKeyword":"Given The user signs in using a valid username and password from the Signin window","stepMatchArguments":[]},{"pwStepLine":26,"gherkinStepLine":22,"keywordType":"Action","textWithKeyword":"When The user attempts to upgrade to a Premium subscription","stepMatchArguments":[]},{"pwStepLine":27,"gherkinStepLine":23,"keywordType":"Outcome","textWithKeyword":"Then The user should be redirected to the checkout page","stepMatchArguments":[]}]},
]; // bdd-data-end